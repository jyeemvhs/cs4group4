...
//        return;
//    }
    let trimTime = req.body.time.trim();
    if (trimTime == "") {
        res.json({error:true});
        return;
    }

    let time = Number(trimTime);
    if (Number.isNaN(time)) {
        res.json({error:true});
        return;
    }

    let obj = new Info(req.user.username,time);
////change code      
    db.postData(obj,res);
...
//    let obj = new Info(identifier,req.body.username,gradeLevel,drive);
////changed code.
//    return(db.putData(obj,res));
//
//});
